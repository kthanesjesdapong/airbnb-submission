...
export const DropDown: FC<DropDownProps> = ({
  ...
  dropDownArrowWidth,
  dropDownArrowDisplay,
}) => {
  return (
    <DropDownWrapper onClick={callBack} className="trigger-button">
      <LinkElement to={to}>{title}</LinkElement>
      <DropDownArrow
        width={dropDownArrowWidth}
        display={dropDownArrowDisplay}
        active={state ? 'active' : undefined}
      />
      <UnorderedList $visible={state} $start={start} $end={end}>
