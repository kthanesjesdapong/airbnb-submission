      ...
      {labelInputs.map(({ inputName, labelTitle, inputType }, i) =>
        formattedErrors && formattedErrors[inputName] !== undefined ? (
          <React.Fragment key={`${labelTitle} fragment error`}>
            <StyledLabel key={`${labelTitle} label ${i}`}>
              {labelTitle}
            </StyledLabel>
            <StyledInput
              key={`${inputName} input ${i}`}
              type={inputType}
              name={inputName}
            />
            <StyledInputError
              key={`${inputName} ${i} INPUT ERROR ${formattedErrors[inputName]}`}
            >
              {formattedErrors[inputName]}
            </StyledInputError>
          </React.Fragment>
        ) : (
