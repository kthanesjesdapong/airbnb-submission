...
export const fetchSingleBusiness = async (
  ...
  query: string,
  businessType: string
): Promise<FormattedSingleBusinessResponse | undefined> => {
  try {
    ...
    const formattedSingleBusinessResponse =
      singleBusinessDataResponse?.data[businessType];
    ...
    const {
      ...
      latitude,
      longitude,
    } = formattedSingleBusinessResponse.data;
