...
export const Slider = () => {
  ...
  const { selected: selectedCategory, prevSelected } = useAppSelector(
    (state) => state.feature
  );

  const activeCategory = featuredCollections.find(
    (featured) => featured.category === selectedCategory
  );
  ...
  } = useNavigatePages(activeCategory!.featured.length);
  ...
  return (
    <SliderContainer>
      ...
      <div className="slider-btns">
        <div className="btn-left" onClick={navigatePrevItem}>
          {'<'}
        </div>
        <div className="btn-right" onClick={navigateNextItem}>
          {'>'}
        </div>
      </div>
    </SliderContainer>
